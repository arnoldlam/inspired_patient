...
def NotesView(request):
	...
	authored_notes = user.authored_notes.filter(~Q(note_type='medication_note') & ~Q(note_type='self_care_note') & ~Q(note_type='appointment_note')).distinct()
	authored_notes = authored_notes.filter(date_accessed__lte=timezone.now()).order_by('-date_accessed')

	notes_read_write = user.notes_read_write.filter(~Q(note_type='medication_note') & ~Q(note_type='self_care_note') & ~Q(note_type='appointment_note')).distinct()
	notes_read_write = notes_read_write.filter(date_accessed__lte=timezone.now()).order_by('-date_accessed')
	
	notes_read_only = user.notes_read_only.filter(~Q(note_type='medication_note') & ~Q(note_type='self_care_note') & ~Q(note_type='appointment_note')).distinct()
...
def SchedulingView(request):
	...
	week_tasks = upcoming_tasks.filter(Q(date_and_time__gte=timezone.now()) & Q(date_and_time__lte=(
		timezone.now() + datetime.timedelta(weeks=1)))).order_by('date_and_time').distinct()
